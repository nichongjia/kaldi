// online2/online-nnet3-decoding-threaded.cc

// Copyright    2013-2014  Johns Hopkins University (author: Daniel Povey)
// Copyright    2016       I2R (author: Chongjia Ni)
...
Nnet3ThreadSynchronizer::Nnet3ThreadSynchronizer():
    abort_(false), 
...
bool Nnet3ThreadSynchronizer::Lock(ThreadType t) {
  if (abort_)
    return false;
  if (t == Nnet3ThreadSynchronizer::kProducer) {
...
bool Nnet3ThreadSynchronizer::UnlockSuccess(ThreadType t) {
  if (t == Nnet3ThreadSynchronizer::kProducer) {
...
bool Nnet3ThreadSynchronizer::UnlockFailure(ThreadType t) {
...
void Nnet3ThreadSynchronizer::SetAbort() {
  abort_ = true;
...
// static
void OnlineNnet3DecodingThreadedConfig::Check() {
...
void SingleUtteranceNnet3DecoderThreaded::AcceptWaveform(
  ...
  if (!waveform_synchronizer_.Lock(Nnet3ThreadSynchronizer::kProducer)) {
  ...
  waveform_synchronizer_.UnlockSuccess(Nnet3ThreadSynchronizer::kProducer);
}

int32 SingleUtteranceNnet3DecoderThreaded::NumWaveformPiecesPending() {
...
int32 SingleUtteranceNnet3DecoderThreaded::NumFramesReceivedApprox() const {
  return num_samples_received_ /
      (sampling_rate_ * feature_pipeline_.FrameShiftInSeconds());
}

void SingleUtteranceNnet3DecoderThreaded::InputFinished() {
...
void SingleUtteranceNnet3DecoderThreaded::TerminateDecoding() {
  bool error = false;
  AbortAllThreads(error);
}

void SingleUtteranceNnet3DecoderThreaded::Wait() {
...
void SingleUtteranceNnet3DecoderThreaded::FinalizeDecoding() {
...
BaseFloat SingleUtteranceNnet3DecoderThreaded::GetRemainingWaveform(
...
void SingleUtteranceNnet3DecoderThreaded::GetAdaptationState(
...
void SingleUtteranceNnet3DecoderThreaded::GetLattice(
...
void SingleUtteranceNnet3DecoderThreaded::GetBestPath(
    bool end_of_utterance,
    Lattice *best_path,
    BaseFloat *final_relative_cost) const {
  // we'll make an exception to the normal const rules, for mutexes, since
...
void SingleUtteranceNnet3DecoderThreaded::AbortAllThreads(bool error) {
  abort_ = true;
  if (error)
    error_ = true;
...
int32 SingleUtteranceNnet3DecoderThreaded::NumFramesDecoded() const {
  const_cast<Mutex&>(decoder_mutex_).Lock();
...
void* SingleUtteranceNnet3DecoderThreaded::RunNnetEvaluation(void *ptr_in) {
  SingleUtteranceNnet3DecoderThreaded *me =
      reinterpret_cast<SingleUtteranceNnet3DecoderThreaded*>(ptr_in);
  try {
...
void* SingleUtteranceNnet3DecoderThreaded::RunDecoderSearch(void *ptr_in) {
  SingleUtteranceNnet3DecoderThreaded *me =
      reinterpret_cast<SingleUtteranceNnet3DecoderThreaded*>(ptr_in);
  try {
...
void SingleUtteranceNnet3DecoderThreaded::WaitForAllThreads() {
...
void SingleUtteranceNnet3DecoderThreaded::ProcessLoglikes(
...
bool SingleUtteranceNnet3DecoderThreaded::FeatureComputation(
  ...
  if (features_done) {
    ...
    if (!waveform_synchronizer_.Lock(Nnet3ThreadSynchronizer::kConsumer)) {
      return false;
    }
    // we've got the lock.
    if (input_waveform_.empty()) {  // we got no data
      if (input_finished_ &&
        ...
        return waveform_synchronizer_.UnlockSuccess(Nnet3ThreadSynchronizer::kConsumer);
      } else {
        // there is no progress.  Unlock with UnlockFailure() so the next call to
        // waveform_synchronizer_.Lock() will lock.
        return waveform_synchronizer_.UnlockFailure(Nnet3ThreadSynchronizer::kConsumer);
      }
...
bool SingleUtteranceNnet3DecoderThreaded::RunNnetEvaluationInternal() {
  ...
  while (true) {
    ...
    if (num_loglike_frames != 0) {  // if we need to output some loglikes...
      while (true) {
        ...
        if (!decodable_synchronizer_.Lock(Nnet3ThreadSynchronizer::kProducer))
          return false;
        ...
        if (num_frames_output - num_frames_decoded <= config_.max_loglikes_copy) {
          ...
          if (!decodable_synchronizer_.UnlockSuccess(Nnet3ThreadSynchronizer::kProducer))
            return false;
          ...
          if (!decodable_synchronizer_.UnlockFailure(Nnet3ThreadSynchronizer::kProducer))
            return false;
        }
      }
    }
...
bool SingleUtteranceNnet3DecoderThreaded::RunDecoderSearchInternal() {
  int32 num_frames_decoded = 0;  // this is just a copy of decoder_->NumFramesDecoded();
  while (true) {  // decode at most one frame each loop.
    if (!decodable_synchronizer_.Lock(Nnet3ThreadSynchronizer::kConsumer))
    ...
    if (decodable_.NumFramesReady() <= num_frames_decoded) {
      ...
      if (decodable_.IsLastFrame(num_frames_decoded - 1)) {
        decodable_synchronizer_.UnlockSuccess(Nnet3ThreadSynchronizer::kConsumer);
        ...
        if (!decodable_synchronizer_.UnlockFailure(Nnet3ThreadSynchronizer::kConsumer))
          return false;
      }
    } else {
      ...
      if (!decodable_synchronizer_.UnlockSuccess(Nnet3ThreadSynchronizer::kConsumer))
        return false;
    }
  }
}

bool SingleUtteranceNnet3DecoderThreaded::EndpointDetected(
