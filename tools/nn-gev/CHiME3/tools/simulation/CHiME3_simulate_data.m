...
addpath ../utils;
upath='G:\CHiME3_Data\data\audio\16kHz\isolated\'; % path to segmented utterances
upath_ext = 'G:\CHiME3_Data\data\audio\16kHz\isolated_ext\';
cpath='G:\CHiME3_Data\data\audio\16kHz\embedded\'; % path to continuous recordings
bpath='G:\CHiME3_Data\data\audio\16kHz\backgrounds\'; % path to noise backgrounds
apath='G:\CHiME3_Data\data\annotations\'; % path to JSON annotations
nchan=6;
...
for set_ind=1:length(sets),
    ...
    for utt_ind=1:length(mat),
        ...
        % Write WAV file
        for c=1:nchan,
            %audiowrite(xsimu(:,c),fs,[udir uname '.CH' int2str(c) '.wav']);
            audiowrite([udir uname '.CH' int2str(c) '.wav',xsimu(:,c),fs]);
            audiowrite([udir_ext uname '.CH' int2str(c) '.Noise.wav'],n(:, c),fs);
        	audiowrite([udir_ext uname '.CH' int2str(c) '.Clean.wav'],ysimu(:, c), fs);
        end
